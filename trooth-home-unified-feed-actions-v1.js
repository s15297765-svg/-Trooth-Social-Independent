...
(function(){
  ...
  function decorate(){
    var root=document.getElementById('troothUnifiedNetworkFeed');if(!root)return;
    root.querySelectorAll('article[data-content-id]').forEach(function(article){
      ...
      if(window.troothSupabase&&window.TroothInteractions){var interactionBox=document.createElement('div');interactionBox.style.marginTop='8px';article.appendChild(interactionBox);window.TroothInteractions.render(window.troothSupabase,null,type,id,interactionBox)}
    });
  }

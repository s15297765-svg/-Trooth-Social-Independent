// Trooth Social Independent — network realtime bridge v4
(function(){
  'use strict';
  ...
  async function handleGroupRequest(payload){const row=payload&&payload.new;if(!row||!currentUser||row.user_id===currentUser.id||row.status!=='pending')return;const g=await sb.from('groups').select('id,name,created_by').eq('id',row.group_id).maybeSingle();if(g.error||!g.data||g.data.created_by!==currentUser.id)return;await notify(g.data.created_by,row.user_id,'group_join_request','👥 Someone requested to join your Trooth Group: '+g.data.name,null,g.data.id,null);refresh('trooth-groups-live')}
  async function handleBusinessPost(payload){const row=payload&&payload.new;if(!row||!currentUser||row.user_id===currentUser.id)return;const b=await sb.from('businesses').select('id,name,user_id').eq('id',row.business_id).maybeSingle();if(b.error||!b.data||b.data.user_id!==currentUser.id)return;await notify(b.data.user_id,row.user_id,'business_post','💼 New activity on your business: '+b.data.name,null,null,b.data.id);refresh('trooth-business-live')}
  async function notify(userId,actorId,kind,body,postId,groupId,businessId){try{let q=sb.from('notifications').select('id').eq('user_id',userId).eq('actor_id',actorId).eq('kind',kind).eq('is_read',false).limit(1);if(postId)q=q.eq('post_id',postId);if(groupId)q=q.eq('group_id',groupId);if(businessId)q=q.eq('business_id',businessId);const existing=await q;if(existing.data&&existing.data.length)return;const row={user_id:userId,actor_id:actorId,kind,body,is_read:false};if(postId)row.post_id=postId;if(groupId)row.group_id=groupId;if(businessId)row.business_id=businessId;const r=await sb.from('notifications').insert(row);if(!r.error){window.dispatchEvent(new CustomEvent('trooth-notifications-refresh'));window.dispatchEvent(new CustomEvent('trooth-unread-refresh'));window.dispatchEvent(new CustomEvent('trooth-activity-live'));}}catch(e){console.warn('Trooth network realtime:',e)}}

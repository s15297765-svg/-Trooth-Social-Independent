// Trooth Social Independent — network realtime bridge v2
(function(){
  'use strict';
  ...
  async function handlePostLike(payload){const row=payload&&payload.new;if(!row||!currentUser||row.user_id===currentUser.id)return;const p=await postOwner(row.post_id);if(!p||p.user_id!==currentUser.id)return;const name=await actorName(row.user_id);await notify(p.user_id,row.user_id,'post_like','❤️ '+name+' liked your Trooth post');refresh('trooth-activity-live')}
  async function handleComment(payload){const row=payload&&payload.new;if(!row||!currentUser||row.user_id===currentUser.id)return;const p=await postOwner(row.post_id);if(!p||p.user_id!==currentUser.id)return;const name=await actorName(row.user_id);await notify(p.user_id,row.user_id,'post_comment','💬 '+name+' commented on your Trooth post');refresh('trooth-activity-live')}
  ...
  async function notify(userId,actorId,kind,body){try{const q=await sb.from('notifications').select('id').eq('user_id',userId).eq('actor_id',actorId).eq('kind',kind).eq('is_read',false).limit(1);if(q.data&&q.data.length)return;const r=await sb.from('notifications').insert({user_id:userId,actor_id:actorId,kind,body,is_read:false});if(!r.error){window.dispatchEvent(new CustomEvent('trooth-notifications-refresh'));window.dispatchEvent(new CustomEvent('trooth-unread-refresh'));}}catch(e){console.warn('Trooth network realtime:',e)}}

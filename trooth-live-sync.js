/* Trooth Social Independent — live notification/message/news/sports sync + breaking/live indicators */
(function(){
  function addStyles(){
  ...
  function indicator(kind,text){
    ...
    if(!el){el=document.createElement('div');el.id='trooth-live-indicator';el.className='trooth-live-indicator';document.body.appendChild(el)}
    el.className='trooth-live-indicator show '+kind;
    el.innerHTML='<span class="trooth-live-dot"></span><span>'+text+'</span>';
    el.onclick=()=>{ if(kind==='sports') location.href='sports.html'; else location.href='news.html'; };
    ...
    sb.auth.getUser().then(({data})=>{
      ...
      const channel=sb.channel('trooth-live-'+user.id)
        ...
        .on('postgres_changes',{event:'*',schema:'public',table:'news_stories'},payload=>{
          ...
          if(payload.eventType==='INSERT') indicator('breaking','🔴 Breaking News — نئی خبر');
        ...
        .on('postgres_changes',{event:'*',schema:'public',table:'sports_stories'},payload=>{
          ...
          if(payload.eventType==='INSERT') indicator('sports','🔵 Live Sports — نئی اپڈیٹ');

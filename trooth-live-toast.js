// Trooth Social Independent — unified live event toasts v4
(function(){
  function boot(){
    ...
    var last='',lastAt=0,timer=null,seq=0;
    function show(text,ttl){var now=Date.now();if(!text||text===last&&now-lastAt<1800)return;last=text;lastAt=now;seq++;var mine=seq,old=document.querySelector('.trooth-live-toast2');if(old){old.classList.add('hide');setTimeout(function(){if(old.parentNode)old.remove()},180)}var x=document.createElement('div');x.className='trooth-live-toast2';x.setAttribute('role','status');x.setAttribute('aria-live','polite');x.setAttribute('aria-atomic','true');x.textContent=text;document.body.appendChild(x);clearTimeout(timer);timer=setTimeout(function(){if(mine===seq&&x.parentNode){x.classList.add('hide');setTimeout(function(){if(x.parentNode)x.remove()},220)}},ttl||2400)}
    ...
    window.addEventListener('trooth-live-refresh-complete',function(){show('🟢 Live feed synced',1200)});
    ...
    document.addEventListener('visibilitychange',function(){if(!document.hidden&&navigator.onLine)show('🟢 Trooth is LIVE',1200)});
    window.addEventListener('pageshow',function(){if(navigator.onLine)show('🟢 Trooth is LIVE',1200)});
  }

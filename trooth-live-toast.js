// Trooth Social Independent — unified live event toasts v3
(function(){
  function boot(){
    ...
    function show(text,ttl){var now=Date.now();if(!text||text===last&&now-lastAt<1800)return;last=text;lastAt=now;seq++;var mine=seq,old=document.querySelector('.trooth-live-toast2');if(old){old.classList.add('hide');setTimeout(function(){if(old.parentNode)old.remove()},180)}var x=document.createElement('div');x.className='trooth-live-toast2';x.setAttribute('role','status');x.setAttribute('aria-live','polite');x.textContent=text;document.body.appendChild(x);clearTimeout(timer);timer=setTimeout(function(){if(mine===seq&&x.parentNode){x.classList.add('hide');setTimeout(function(){if(x.parentNode)x.remove()},220)}},ttl||2400)}

...
(function(){
  ...
  let channel=null,authSub=null,reconnectTimer=null,started=false,stopped=false;
  ...
  function cleanup(){
    stopped=true;clearTimeout(reconnectTimer);reconnectTimer=null;
  ...
  function boot(reconnectMode){
    const sb=window.troothSupabase;if(!sb||stopped)return;
    if(reconnectMode&&channel){try{sb.removeChannel(channel)}catch(e){}channel=null;}
  ...
  function start(){if(started)return;started=true;stopped=false;boot(false);const sb=window.troothSupabase;if(sb&&sb.auth){const result=sb.auth.onAuthStateChange(event=>{if(event==='SIGNED_OUT'||event==='USER_DELETED')cleanup();else if(event==='SIGNED_IN'||event==='TOKEN_REFRESHED'||event==='USER_UPDATED'){stopped=false;boot(true)}});authSub=result&&result.data&&result.data.subscription?result.data.subscription:null}window.addEventListener('beforeunload',cleanup,{once:true})}

/* Trooth Social Independent — unified Home Experience bridge v5 */
(function(){
  if(window.__troothHomeUnifiedBridgeV5)return;
  window.__troothHomeUnifiedBridgeV5=true;
  let lastToast={};
  let toastTimer=null;
  function toast(text,href,key){
    ...
    if(!e){
      e=document.createElement('button');e.type='button';e.id='trooth-home-live';
      e.setAttribute('role','status');e.setAttribute('aria-live','polite');e.setAttribute('aria-atomic','true');
      e.style='position:fixed;top:214px;right:14px;z-index:10001;background:#fff;border:1px solid #bbf7d0;border-radius:14px;padding:10px 14px;box-shadow:0 8px 24px #0002;font:700 13px system-ui;color:#166534;max-width:calc(100vw - 28px);text-align:left';
      e.addEventListener('click',function(){if(e.dataset.href)location.href=e.dataset.href});
      document.body.appendChild(e);
    }
    e.textContent=text;e.dataset.href=href||'';e.title=href?'Open '+text.replace(/^\S+\s*/,''):text;e.setAttribute('aria-label',href?text+' — open':text);
    e.style.cursor=href?'pointer':'default';e.style.pointerEvents=href?'auto':'none';
  ...
  function start(){
    if(window.__troothHomeUnifiedBridgeV5Started)return;
    window.__troothHomeUnifiedBridgeV5Started=true;

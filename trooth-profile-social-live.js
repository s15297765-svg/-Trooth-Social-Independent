...
(function(){
  function boot(){
    ...
    async function loadPeople(){
      if(!uid)return;if(loading){pending=true;return}loading=true;
      try{
        ...
        var followers=rows.filter(x=>x.following_id===uid).map(x=>map.get(x.follower_id)).filter(Boolean),following=rows.filter(x=>x.follower_id===uid).map(x=>map.get(x.following_id)).filter(Boolean);
        var fr=await sb.from('friend_requests').select('sender_id,receiver_id,status').or('sender_id.eq.'+uid+',receiver_id.eq.'+uid).eq('status','accepted');
        var fids=[...new Set((fr.data||[]).map(x=>x.sender_id===uid?x.receiver_id:x.sender_id))];
        var fp=fids.length?await sb.from('profiles').select('id,display_name,bio,avatar_url').in('id',fids):{data:[]};
        render('followers',followers);render('following',following);render('friends',fp.data||[]);
      }finally{loading=false;if(pending){pending=false;scheduleLoad(60)}}
    }

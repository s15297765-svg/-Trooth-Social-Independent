// Trooth Social Independent — unified Home post interactions v2
(function(){
  ...
  async function enhancePost(post,sb,user){
    ...
    async function notify(kind,body){if(!user)return;var r=await sb.from('posts').select('user_id').eq('id',id).maybeSingle();var ownerId=r.data&&r.data.user_id;if(ownerId&&ownerId!==user.id)await sb.from('notifications').insert({user_id:ownerId,actor_id:user.id,kind:kind,body:body,is_read:false})}
    actions.querySelector('[data-like]').onclick=async function(){if(!user){location.href='auth.html';return}this.disabled=true;try{var r=liked?await sb.from('post_likes').delete().eq('post_id',id).eq('user_id',user.id):await sb.from('post_likes').insert({post_id:id,user_id:user.id});if(r.error)throw r.error;await sync();if(liked){}else await notify('Like','Someone liked your Trooth post.');toast(liked?'Like removed':'❤️ Liked')}catch(e){toast(e.message||'Like failed')}finally{this.disabled=false}};
  ...
  function bootRealtime(){
    ...
    sb.channel('trooth-home-post-unified-live').on('postgres_changes',{event:'*',schema:'public',table:'post_likes'},function(e){var id=e.new&&e.new.post_id||e.old&&e.old.post_id;var p=document.querySelector('.post[data-post-id="'+id+'"]');if(p&&p.__troothHomePostSync)p.__troothHomePostSync()}).on('postgres_changes',{event:'*',schema:'public',table:'comments'},function(e){var id=e.new&&e.new.post_id||e.old&&e.old.post_id;var p=document.querySelector('.post[data-post-id="'+id+'"]');if(p&&p.__troothHomePostSync)p.__troothHomePostSync()}).subscribe();
  }

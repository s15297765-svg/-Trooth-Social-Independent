// Trooth Social Independent — unified Home post interactions v3
(function(){
  ...
  async function enhancePost(post,sb,user){
    ...
    function render(){
      var lc=actions.querySelector('[data-like-count]'),cc=actions.querySelector('[data-comment-count]'),lb=actions.querySelector('[data-like]');
      if(lb)lb.innerHTML=(liked?'❤️ Liked':'🤍 Like')+' <span data-like-count>('+likes.length+')</span>';
      if(cc)cc.textContent='('+comments.length+')';
      commentsBox.innerHTML=comments.map(function(x){return '<div style="padding:8px 4px;border-top:1px solid #e5eee8">💬 '+esc(x.body)+'</div>'}).join('')||'<small style="color:#718276">No comments yet.</small>';
    }
    render();
    async function sync(){
    ...
    async function activityRefresh(){
      window.dispatchEvent(new CustomEvent('trooth-post-activity-refresh',{detail:{postId:id}}));
    }
    actions.querySelector('[data-like]').onclick=async function(){if(!user){location.href='auth.html';return}this.disabled=true;try{var r=liked?await sb.from('post_likes').delete().eq('post_id',id).eq('user_id',user.id):await sb.from('post_likes').insert({post_id:id,user_id:user.id});if(r.error)throw r.error;await sync();await activityRefresh();toast(liked?'Like removed':'❤️ Liked')}catch(e){toast(e.message||'Like failed')}finally{this.disabled=false}};
  ...
  function bootRealtime(){
    ...
    sb.channel('trooth-home-post-unified-live').on('postgres_changes',{event:'*',schema:'public',table:'post_likes'},function(e){var id=e.new&&e.new.post_id||e.old&&e.old.post_id;var p=document.querySelector('.post[data-post-id="'+id+'"]');if(p&&p.__troothHomePostSync)p.__troothHomePostSync()}).on('postgres_changes',{event:'*',schema:'public',table:'comments'},function(e){var id=e.new&&e.new.post_id||e.old&&e.old.post_id;var p=document.querySelector('.post[data-post-id="'+id+'"]');if(p&&p.__troothHomePostSync)p.__troothHomePostSync()}).on('postgres_changes',{event:'*',schema:'public',table:'post_shares'},function(e){window.dispatchEvent(new CustomEvent('trooth-post-share-live',{detail:{postId:e.new&&e.new.post_id||e.old&&e.old.post_id}}))}).subscribe();
  }

// Trooth — profile social actions bridge v3
(function(){
  function boot(){
    var sb=window.troothSupabase;if(!sb||window.__troothProfileSocialActionsV3)return;window.__troothProfileSocialActionsV3=true;
    ...
    async function render(){
      ...
      try{
        ...
        var ub=host.querySelector('[data-act=unfriend]');if(ub)ub.onclick=async function(){ub.disabled=true;try{var r=await sb.from('friend_requests').delete().eq('status','accepted').or('and(sender_id.eq.'+u.id+',receiver_id.eq.'+target+'),and(sender_id.eq.'+target+',receiver_id.eq.'+u.id+')');if(r.error)throw r.error;window.dispatchEvent(new CustomEvent('trooth-social-graph-refresh'))}catch(e){console.warn('Trooth unfriend:',e)}finally{ub.disabled=false;render()}};

...
(function(){
  function boot(){
    ...
    window.troothProfileSocialActions=async function(targetId,host){
      ...
      box.querySelector('[data-act=follow]').onclick=async()=>{if(following)await sb.from('connections').delete().eq('follower_id',uid.id).eq('following_id',targetId);else await sb.from('connections').insert({follower_id:uid.id,following_id:targetId});await window.troothProfileSocialActions(targetId,box)};
      box.querySelector('[data-act=friend]').onclick=async()=>{if(friend||request)return;await sb.from('friend_requests').insert({sender_id:uid.id,receiver_id:targetId,status:'pending'});await window.troothProfileSocialActions(targetId,box)};

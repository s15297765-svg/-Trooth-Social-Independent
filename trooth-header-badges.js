...
(function(){
  ...
  function boot(){
    ...
    var timer=null,lastMsg=-1,lastNote=-1;
    ...
    async function refresh(){var sb=window.troothSupabase;if(!sb||!navigator.onLine)return;try{var x=await sb.auth.getUser(),u=x.data&&x.data.user;if(!u){targets('msg').forEach(function(e){addBadge(e,0)});targets('note').forEach(function(e){addBadge(e,0)});return}var r=await Promise.all([sb.from('messages').select('id',{count:'exact',head:true}).eq('receiver_id',u.id).eq('is_read',false),sb.from('notifications').select('id',{count:'exact',head:true}).eq('user_id',u.id).eq('is_read',false)]);var m=r[0].count||0,n=r[1].count||0;targets('msg').forEach(function(e){addBadge(e,m)});targets('note').forEach(function(e){addBadge(e,n)});if(m!==lastMsg||n!==lastNote)window.dispatchEvent(new CustomEvent('trooth-header-badges-updated',{detail:{messages:m,notifications:n}}));lastMsg=m;lastNote=n}catch(e){}}
    function schedule(ms){clearTimeout(timer);timer=setTimeout(refresh,ms||250)}
    window.refreshTroothHeaderBadges=refresh;
    window.addEventListener('trooth-messages-refresh',function(){schedule(100)});window.addEventListener('trooth-notifications-refresh',function(){schedule(100)});window.addEventListener('trooth-auth-changed',function(){schedule(200)});window.addEventListener('online',function(){schedule(300)});window.addEventListener('visibilitychange',function(){if(!document.hidden)schedule(300)});
    new MutationObserver(function(){schedule(350)}).observe(document.body,{childList:true,subtree:true});setTimeout(refresh,2200);setInterval(function(){if(!document.hidden)refresh()},45000);
  }

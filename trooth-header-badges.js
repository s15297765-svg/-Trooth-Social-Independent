...
(function(){
  ...
  function boot(){
    ...
    function refresh(){var sb=window.troothSupabase;if(!sb)return;sb.auth.getUser().then(function(x){var u=x.data&&x.data.user;if(!u)return Promise.all([sb.from('messages').select('id',{count:'exact',head:true}).eq('receiver_id',u.id).eq('is_read',false),sb.from('notifications').select('id',{count:'exact',head:true}).eq('user_id',u.id).eq('is_read',false)]).then(function(r){targets('msg').forEach(function(e){addBadge(e,r[0].count||0)});targets('note').forEach(function(e){addBadge(e,r[1].count||0)})})})}
    window.addEventListener('trooth-messages-refresh',refresh);window.addEventListener('trooth-notifications-refresh',refresh);window.addEventListener('online',refresh);setTimeout(refresh,2600);setInterval(refresh,30000);
  }

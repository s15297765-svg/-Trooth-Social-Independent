// Trooth Social Independent — Feed interactions enhancement v4
(function () {
  ...
  function polish(){
    if(document.getElementById('trooth-feed-v4-style')) return;
    const st=document.createElement('style');st.id='trooth-feed-v4-style';st.textContent=`
  ...
  async function notifyPostOwner(s, postId, actorId, kind, body) {
    try { const { data: post } = await s.from('posts').select('user_id').eq('id', postId).maybeSingle(); if (!post?.user_id || post.user_id === actorId) return; await s.from('notifications').insert({ user_id: post.user_id, actor_id: actorId, kind, body, is_read: false }); } catch (_) {}
  }
  async function hydrateFeed() {
    ...
    const [likesRes, commentsRes, savesRes] = await Promise.all([
      ...
      s.from('saved_content').select('id,content_id').eq('content_type','post').in('content_id', ids)
    ]);
    const likes = likesRes.data || [], comments = commentsRes.data || [], saves = savesRes.data || []; let me = null;
    try { me = (await s.auth.getUser()).data.user || null; } catch (_) {}
    const counts = {}, commentCounts = {}; likes.forEach(l => { counts[l.post_id] = (counts[l.post_id] || 0) + 1; }); comments.forEach(c => { commentCounts[c.post_id] = (commentCounts[c.post_id] || 0) + 1; });
  ...
  window.toggleLike = async function (id) { const s = await wait(), me = (await s.auth.getUser()).data.user; if (!me) { alert('Please login first.'); location.href = 'auth.html'; return; } const q = await s.from('post_likes').select('post_id').eq('post_id', id).eq('user_id', me.id).maybeSingle(); if (q.error) { alert(q.error.message); return; } if (q.data) { const r = await s.from('post_likes').delete().eq('post_id', id).eq('user_id', me.id); if (r.error) { alert(r.error.message); return; } } else { const r = await s.from('post_likes').insert({ post_id: id, user_id: me.id }); if (r.error) { alert(r.error.message); return; } await notifyPostOwner(s, id, me.id, 'Like', 'liked your post on Trooth.'); } await hydrateFeed(); };
  window.addComment = async function (id) { const s = await wait(), me = (await s.auth.getUser()).data.user; if (!me) { alert('Please login first.'); location.href = 'auth.html'; return; } const v = prompt('Write your comment:'); if (!v || !v.trim()) return; const r = await s.from('comments').insert({ post_id: id, user_id: me.id, body: v.trim() }); if (r.error) { alert(r.error.message); return; } await notifyPostOwner(s, id, me.id, 'Comment', 'commented on your post on Trooth.'); await hydrateFeed(); };
  window.sharePost = async function (id) { const s = await wait(), me = (await s.auth.getUser()).data.user; if (!me) { alert('Please login first.'); location.href = 'auth.html'; return; } const url = location.origin + location.pathname + '#post-' + id; try { if (navigator.share) await navigator.share({ title: 'Trooth Social Independent', text: 'Check this post on Trooth', url }); else { await navigator.clipboard.writeText(url); alert('Post link copied!'); } await notifyPostOwner(s, id, me.id, 'Share', 'shared your post on Trooth.'); } catch (_) {} };
  window.toggleSave = async function (id) { const s = await wait(), me = (await s.auth.getUser()).data.user; if (!me) { alert('Please login first.'); location.href = 'auth.html'; return; } const q = await s.from('saved_content').select('id').eq('content_type','post').eq('content_id',id).eq('user_id',me.id).maybeSingle(); if(q.error){alert(q.error.message);return;} if(q.data){const r=await s.from('saved_content').delete().eq('id',q.data.id).eq('user_id',me.id);if(r.error){alert(r.error.message);return;} alert('Removed from Saved ✓');}else{const r=await s.from('saved_content').insert({user_id:me.id,content_type:'post',content_id:id});if(r.error){alert(r.error.message);return;} alert('Saved to your Trooth ✓');} window.dispatchEvent(new CustomEvent('trooth-saved-content-refresh')); await hydrateFeed(); };

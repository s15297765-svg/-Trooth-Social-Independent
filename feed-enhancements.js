// Trooth Social Independent — Feed interactions enhancement v3
(function () {
  ...
  function polish(){
    if(document.getElementById('trooth-feed-v3-style')) return;
    const st=document.createElement('style');st.id='trooth-feed-v3-style';st.textContent=`
      ...
      .postActions .like-action{color:#237b53}
  ...
  async function hydrateFeed() {
    ...
    const likes = likesRes.data || [], comments = commentsRes.data || []; let me = null;
    ...
    posts.forEach(post => {
      ...
      actions.innerHTML = `<button class="action like-action" onclick="toggleLike('${id}',this)">${mine.has(id) ? '❤️ Liked' : '👍 Like'} <span>${counts[id] || 0}</span></button><button class="action" onclick="addComment('${id}')">💬 Comment <span>${commentCounts[id] || 0}</span></button><button class="action" onclick="sharePost('${id}')">↗ Share</button>`;

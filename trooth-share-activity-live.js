...
(function(){
  function boot(){
    ...
    function postId(post){
      var direct=post.getAttribute('data-post-id');
    ...
    async function refreshShares(id,post){
      ...
      if(!el){
        el=document.createElement('span');el.className='trooth-share-count';
        var bar=post.querySelector('.trooth-reaction-bar')||post.querySelector('.postactions');
        if(bar)bar.parentNode.insertBefore(el,bar.nextSibling);
      }
      el.textContent='↗ '+((r.data||[]).length)+' shares';
    }
    async function share(id,post){
      ...
      try{
        if(navigator.share)await navigator.share({title:'Trooth Social Independent',text:'Check this post on Trooth',url:url});
        else if(navigator.clipboard){await navigator.clipboard.writeText(url);alert('Post link copied!');}
        else prompt('Copy post link:',url);
      }catch(e){}
      window.dispatchEvent(new CustomEvent('trooth-post-shared',{detail:{postId:id}}));
      window.dispatchEvent(new CustomEvent('trooth-navigation-refresh',{detail:{type:'share',postId:id}}));
    }
    function decorate(){
      feed.querySelectorAll('.post').forEach(function(post){
        ...
        if(!post.querySelector('.trooth-share-live')){
          ...
          var b=document.createElement('button');b.className='trooth-share-live';b.type='button';b.textContent='↗ Share';b.setAttribute('aria-label','Share this post');b.onclick=function(){share(id,post)};actions.appendChild(b);
        }
    ...
    var timer=null;
    new MutationObserver(function(){clearTimeout(timer);timer=setTimeout(decorate,60);}).observe(feed,{childList:true,subtree:true});
    sb.channel('trooth-post-shares-live').on('postgres_changes',{event:'*',schema:'public',table:'post_shares'},function(e){
      var id=e.new?.post_id||e.old?.post_id;if(!id)return;
      feed.querySelectorAll('.post').forEach(function(post){if(postId(post)===id)refreshShares(id,post);});
    }).subscribe();
    sb.auth.onAuthStateChange(function(_,session){user=session?.user||null;decorate();});

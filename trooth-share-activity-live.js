// Trooth Social Independent — Live Share + Activity v2
(function(){
  function boot(){
    ...
    var user=null,channel=null,timer=null,authSub=null,refreshing=new Set(),recentShares=new Map();
    ...
    function prune(){var now=Date.now();recentShares.forEach(function(t,k){if(now-t>DEDUPE_TTL)recentShares.delete(k)})}
    async function refreshShares(id,post){
      if(!id||!post||refreshing.has(id)||!navigator.onLine)return;refreshing.add(id);
    ...
    async function share(id,post){
      ...
      if(!navigator.onLine){alert('You are offline. Please reconnect and try again.');return}
      prune();var key=user.id+':'+id;if(recentShares.has(key)){await refreshShares(id,post);return}
      recentShares.set(key,Date.now());
      var r=await sb.from('post_shares').insert({post_id:id,user_id:user.id});
      if(r.error&&r.error.code!=='23505'){recentShares.delete(key);console.error(r.error);alert('Share failed. Please try again.');return}
    ...
    function subscribe(){if(channel||!user||!navigator.onLine)return;channel=sb.channel('trooth-post-shares-live-'+user.id+'-'+Date.now()).on('postgres_changes',{event:'*',schema:'public',table:'post_shares'},function(e){var id=e.new?.post_id||e.old?.post_id;if(!id)return;feed.querySelectorAll('.post').forEach(function(post){if(postId(post)===id)refreshShares(id,post)});window.dispatchEvent(new CustomEvent('trooth-share-live',{detail:{postId:id,event:e.eventType}}))}).subscribe(function(status){if(status==='SUBSCRIBED')window.dispatchEvent(new CustomEvent('trooth-realtime-connected',{detail:{source:'share-activity'}}));else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')window.dispatchEvent(new CustomEvent('trooth-realtime-status',{detail:{status:'retrying',source:'share-activity'}}))});window.troothShareActivityLiveChannel=channel}
    async function syncAuth(){var r=await sb.auth.getUser();user=r.data&&r.data.user||null;if(user)subscribe();else cleanup();decorate()}
    new MutationObserver(function(){clearTimeout(timer);timer=setTimeout(decorate,80)}).observe(feed,{childList:true,subtree:true});
    syncAuth();
    ...
    window.addEventListener('online',function(){subscribe();decorate()});
    window.addEventListener('offline',function(){if(channel){try{sb.removeChannel(channel)}catch(e){}channel=null}window.troothShareActivityLiveChannel=null});

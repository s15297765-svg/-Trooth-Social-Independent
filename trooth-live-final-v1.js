// Trooth Social Independent — visible reference-style live layer v4
(function(){
  'use strict';
  if(window.__troothLiveFinalV4)return; window.__troothLiveFinalV4=true;
  var KEY='trooth_live_posts_v4';
  var seed=[
  ...
  function init(){
    ...
    var feed=document.getElementById('feed');if(!feed)return;
    setTimeout(function(){
      var title=document.getElementById('trooth-demo-title');
      if(!title){title=document.createElement('div');title.id='trooth-demo-title';title.className='trooth-demo-title';title.textContent='🌿 TROOTH MULTIMEDIA DEMO — PHOTO • VIDEO • AUDIO';feed.parentNode.insertBefore(title,feed);}
      render();
    },1000);
    ...
    var post=document.querySelector('[onclick="publishPost()"]');if(post)post.addEventListener('click',function(){setTimeout(function(){if(!feed.querySelector('.trooth-live-post'))publishFallback()},700)});
  }

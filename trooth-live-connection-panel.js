// Trooth Social Independent — unified realtime connection panel v8
(function(){
  function boot(){
    ...
    var last='',state='connecting',timer=null,moduleErrors=0,lastConnectedAt=0;
    ...
    function temporaryConnecting(){
      ...
      if(state==='connected'&&Date.now()-lastConnectedAt<8000)return;
      set('connecting');
      timer=setTimeout(function(){if(online()&&state==='connecting')set('retrying',{attempt:1})},5000);
    }
    window.addEventListener('trooth-realtime-status',function(e){var s=e.detail&&e.detail.status;if(s==='SUBSCRIBED')set('connected');else if(s==='CHANNEL_ERROR'||s==='TIMED_OUT'||s==='RETRYING')set('retrying',e.detail);else if(s==='CLOSED'||s==='RECONNECTING')set('reconnecting');else if(s==='OFFLINE')set('offline');else if(s==='SIGNED_OUT')set('offline');else if(s==='CONNECTING')temporaryConnecting()});

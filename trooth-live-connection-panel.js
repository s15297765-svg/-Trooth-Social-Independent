...
(function(){
  function boot(){
    ...
    var last='',timer=null,moduleErrors=0;
    function set(state,detail){
      ...
      if(moduleErrors&&state==='connected'){title+=' • '+moduleErrors+' live module load error'+(moduleErrors===1?'':'s')}
    ...
    function temporaryConnecting(){
      ...
      set('connecting');
      timer=setTimeout(function(){if(online()&&last==='🟡 Connecting…')set('retrying',{attempt:1})},5000);
    }
    ...
    window.addEventListener('pageshow',function(){temporaryConnecting()});

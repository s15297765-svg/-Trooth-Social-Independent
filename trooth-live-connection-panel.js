...
(function(){
  function boot(){
    ...
    function set(state){
      ...
      else if(state==='retrying'){el.textContent='🟠 Retrying…';el.style.color='#a55b16';el.title='Trooth is retrying the realtime connection'}
      else {el.textContent='⚪ Reconnecting…';el.style.color='#718276';el.title='Realtime connection is being restored'}
    }
    window.addEventListener('trooth-realtime-status',function(e){var s=e.detail&&e.detail.status;if(s==='SUBSCRIBED')set('connected');else if(s==='CHANNEL_ERROR'||s==='TIMED_OUT')set('retrying');else set('connecting')});

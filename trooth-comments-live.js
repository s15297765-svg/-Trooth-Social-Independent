// Trooth Social Independent — Live Comment Composer v2
(function(){
  if(window.__troothCommentsLiveV2)return;window.__troothCommentsLiveV2=true;
  function boot(){
    var sb=window.troothSupabase;if(!sb)return setTimeout(boot,300);
    var feed=document.getElementById('feed');if(!feed)return;
    var user=null;
    function getId(post){return post.getAttribute('data-post-id')||post.getAttribute('data-post')||((post.querySelector('[onclick*="likePost"]')?.getAttribute('onclick')||'').match(/likePost\('([^']+)'\)/)||[])[1]||null;}
    async function addComment(id,input){
      ...
      if(r.error){console.error(r.error);alert(r.error.message||'Comment failed.');return;}
      input.value='';window.dispatchEvent(new CustomEvent('trooth-comment-added',{detail:{postId:id}}));
      var box=document.getElementById('comments-'+id);if(box&&typeof window.loadComments==='function')window.loadComments(id);
    }
    function decorate(){
      feed.querySelectorAll('.post').forEach(function(post){
        ...
        btn.onclick=function(){addComment(id,input)};
        input.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();addComment(id,input)}});
        var comments=post.querySelector('#comments-'+id)||post.querySelector('.comments')||post.querySelector('.trooth-comments');

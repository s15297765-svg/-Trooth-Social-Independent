// Trooth Social Independent — end-to-end flow guard v3
(function(){
  if(window.__troothE2EFlowGuardV3)return;
  window.__troothE2EFlowGuardV3=true;
  function boot(){
    var path=(location.pathname||'').split('/').pop().toLowerCase();
    var qs=new URLSearchParams(location.search);
    var routes={
      signup:'auth.html',profile:'auth.html',friends:'friends.html',chat:'chat.html',
      notifications:'notifications.html',news:'news.html',sports:'sports.html',
    ...
    function safeOpen(key,id,param){if(current(key)&&!id)return false;return go(key,id,param)}
    window.TroothE2EFlow={
      routes:routes,page:path,query:qs,go:go,safeOpen:safeOpen,
      openProfile:function(id){return go('profile',id,'profile')},
    ...
    window.dispatchEvent(new CustomEvent('trooth-e2e-flow-ready',{detail:{page:path,routes:routes,version:3}}));
  }

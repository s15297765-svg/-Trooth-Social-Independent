...
(function(){
  ...
  function patch(){
    ...
    if(loginBtn){loginBtn.textContent='Login with Email';loginBtn.onclick=async function(){const email=emailInput.value.trim().toLowerCase(),password=(pass&&pass.value)||'';if(!email||!email.includes('@'))return status('براہِ کرم درست Email درج کریں۔');if(password.length<6)return status('Password کم از کم 6 حروف کا ہونا چاہیے۔');loginBtn.disabled=true;status('Email Login ہو رہا ہے…');try{const {error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;location.href='index.html';}catch(e){status(e.message||'Login ناکام ہوگیا۔');}finally{loginBtn.disabled=false;}};}
    if(signupBtn){signupBtn.textContent='Create Account with Email';signupBtn.onclick=async function(){const email=emailInput.value.trim().toLowerCase(),password=(pass&&pass.value)||'',displayName=(name&&name.value.trim())||'';if(!displayName)return status('براہِ کرم اپنا نام درج کریں۔');if(!email||!email.includes('@'))return status('براہِ کرم درست Email درج کریں۔');if(password.length<6)return status('Password کم از کم 6 حروف کا ہونا چاہیے۔');signupBtn.disabled=true;status('Email account بنایا جا رہا ہے…');try{const {data,error}=await sb.auth.signUp({email,password,options:{data:{display_name:displayName}}});if(error)throw error;if(data.session){location.href='index.html';}else status('Account بن گیا ہے۔ Email confirmation مکمل کرکے پھر Login کریں۔');}catch(e){status(e.message||'Account نہیں بن سکا۔');}finally{signupBtn.disabled=false;}};}
  }

// Trooth Social Independent — profile/post/notification deep-link v2
(function(){
  if(window.__troothProfileDeepLinkV2)return;
  window.__troothProfileDeepLinkV2=true;
  ...
  function publicProfile(){
    ...
    function start(){
      ...
      sb.auth.getUser().then(async r=>{
        ...
        app.innerHTML='<div class="card"><div class="cover" style="'+(x.cover_url?'background-image:url(\"'+esc(x.cover_url)+'\")':'')+'"><div class="avatar">'+(x.avatar_url?'<img src="'+esc(x.avatar_url)+'" alt="Profile photo">':initial)+'</div></div><div class="profileHead"><h1>'+esc(x.display_name||'Trooth User')+'</h1><p class="muted">'+esc(x.bio||'Trooth Member • Independent Social & Business Network')+'</p><div class="stats"><div class="stat"><b>'+posts.length+'</b><br>Posts</div></div></div><div class="quick"><a href="index.html">🏠 Home</a><a href="friends.html">👥 Friends</a><a href="chat.html">💬 Messages</a></div></div><div class="card"><h3>📝 Posts</h3>'+(posts.length?posts.map(q=>'<div class="post" data-post="'+esc(q.id)+'"><b>📝 '+esc(x.display_name||'Trooth User')+'</b><p>'+esc(q.body||'')+'</p>'+(q.media_url?(String(q.media_type||'').startsWith('video')?'<video src="'+esc(q.media_url)+'" controls style="max-width:100%;border-radius:10px"></video>':'<img src="'+esc(q.media_url)+'" style="max-width:100%;border-radius:10px">'):'')+'<br><small class="muted">'+new Date(q.created_at).toLocaleString()+'</small></div>').join(''):'<p class="muted">No posts yet.</p>')+'</div>';
  ...
  function notificationRoutes(){
    ...
    open.__troothV2=true;window.openNotification=open;
  }

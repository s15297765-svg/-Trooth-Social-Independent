// Trooth Social Independent — Friends + Profile + Chat connected flow v6
(function(){
  if(window.__troothSocialConnectionFlowV6)return;
  window.__troothSocialConnectionFlowV6=true;
  function boot(){
    ...
    document.addEventListener('click',function(e){
      ...
      if(!el||!route(el))return;e.preventDefault();
    },true);
    ...
    if(path==='friends.html'&&qs.get('chat')){
      var id=qs.get('chat');
      setTimeout(function(){openFriendChat(id)},350);setTimeout(function(){openFriendChat(id)},1000);
    }
    if(path==='chat.html'&&qs.get('user')){
      var chatId=qs.get('user');
      function openDirect(){
        try{
          if(typeof window.openChatWithUser==='function')window.openChatWithUser(chatId);
          else if(typeof window.selectUser==='function')window.selectUser(chatId);
        }catch(e){}
      }
      setTimeout(openDirect,350);setTimeout(openDirect,1000);
    }

// Trooth Social Independent — interaction reliability + mobile UX bridge v5
(function(){
  if(window.__troothInteractionReliabilityV5)return;window.__troothInteractionReliabilityV5=true;
  function toast(msg){
  ...
  function boot(){
    if(window.__troothInteractionReliabilityBootedV5)return;window.__troothInteractionReliabilityBootedV5=true;
    var sb=window.troothSupabase;if(!sb||!window.TroothInteractions)return;
    var sharing=false,lastShareUrl='',lastShareAt=0;
    document.addEventListener('click',function(e){
      var b=e.target.closest&&e.target.closest('[data-trooth-share]');
      if(!b)return;
      var url=b.getAttribute('data-trooth-share');if(!url)return;
      var now=Date.now();
      if(sharing||(url===lastShareUrl&&now-lastShareAt<1200)){e.preventDefault();e.stopPropagation();return}
      lastShareUrl=url;lastShareAt=now;
      if(navigator.share){
        e.preventDefault();sharing=true;b.setAttribute('aria-busy','true');b.setAttribute('aria-disabled','true');
        ...
        }).finally(function(){sharing=false;b.removeAttribute('aria-busy');b.removeAttribute('aria-disabled')});
      }else{

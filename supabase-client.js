...
(function(){
  ...
  function loadNext(){
    if(i>=scripts.length){
      if(!window.troothSupabase)dispatch('trooth-supabase-error',{error:new Error('Supabase client could not be loaded from the available CDNs')});
      return;
    }
    ...
    s.async=false;
    s.onload=function(){
      if(isSupabaseCdn(src) && window.supabase && window.supabase.createClient && !window.troothSupabase){
        try{

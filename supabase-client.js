...
(function(){
  ...
  script.onload=function(){
    ...
    const files=['feed-enhancements.js','trooth-live-sync.js','trooth-stories-live.js','trooth-content-hub.js','trooth-groups-home.js','trooth-groups-enhancement.js'];
    let i=0;
    function next(){
      if(i>=files.length){window.dispatchEvent(new Event('trooth-supabase-ready'));return}
      const s=document.createElement('script');s.src=files[i++];s.onload=next;s.onerror=next;document.body.appendChild(s)
    }
    next();
  };

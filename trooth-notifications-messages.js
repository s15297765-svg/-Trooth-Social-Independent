...
(function(){
  function boot(){
    ...
    function channels(){if(!user)return;try{if(notifChannel)sb.removeChannel(notifChannel);if(msgChannel)sb.removeChannel(msgChannel)}catch(e){}
      notifChannel=sb.channel('trooth-notifications-'+user.id).on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:'user_id=eq.'+user.id},function(p){refresh();if(p.eventType==='INSERT'){toast('🔔 نئی Notification','notifications.html');window.dispatchEvent(new CustomEvent('trooth-notification-incoming',{detail:p.new}))}}).subscribe();

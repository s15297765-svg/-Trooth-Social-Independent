...
(function(){
  function boot(){
    ...
    function channels(){if(!user)return;try{if(notifChannel)sb.removeChannel(notifChannel);if(msgChannel)sb.removeChannel(msgChannel)}catch(e){}
      notifChannel=sb.channel('trooth-notifications-'+user.id).on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:'user_id=eq.'+user.id},function(p){refresh(false);if(p.eventType==='INSERT'){const n=p.new||{};const url=(n.post_id?('index.html?post='+encodeURIComponent(n.post_id)):(n.kind==='message'?'chat.html':'notifications.html'));toast('🔔 نئی Notification',url);window.dispatchEvent(new CustomEvent('trooth-notification-incoming',{detail:n}))}}).subscribe();
      msgChannel=sb.channel('trooth-messages-'+user.id).on('postgres_changes',{event:'*',schema:'public',table:'messages',filter:'receiver_id=eq.'+user.id},function(p){refreshMessages(false);if(p.eventType==='INSERT'){toast('💬 نیا Message','chat.html');window.dispatchEvent(new CustomEvent('trooth-message-incoming',{detail:p.new}))}}).subscribe();
    }
    async function start(){const r=await sb.auth.getUser();user=r.data&&r.data.user;if(!user)return;await refresh(false);await refreshMessages(false);channels()}
    ...
    window.refreshTroothNotifications=()=>refresh(true);window.refreshTroothMessages=()=>refreshMessages(true);
    window.addEventListener('trooth-notifications-refresh',function(e){if(user&&(!e.detail||e.source!=='trooth-notifications-messages'))refresh(false)});window.addEventListener('trooth-messages-refresh',function(e){if(user&&(!e.detail||e.source!=='trooth-notifications-messages'))refreshMessages(false)});window.addEventListener('trooth-auth-changed',function(){start()});window.addEventListener('online',function(){start()});
    start();
  }

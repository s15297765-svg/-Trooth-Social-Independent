...
(function(){
  function boot(){
    ...
    function show(n){n=Math.max(0,Number(n)||0);var b=badge();b.textContent=n>99?'99+':n;b.style.display=n?'block':'none';if(n!==last){last=n;window.dispatchEvent(new CustomEvent('trooth-unread-count',{detail:{count:n}}))}}
    function save(n){try{localStorage.setItem(key,String(n));localStorage.setItem(key+'_at',String(Date.now()))}catch(e){}}
    function refresh(){var sb=window.troothSupabase;if(!sb||!navigator.onLine)return;sb.auth.getUser().then(function(r){var u=r.data&&r.data.user;if(!u){show(0);return null}return sb.from('messages').select('id',{count:'exact',head:true}).eq('receiver_id',u.id).eq('is_read',false).then(function(x){if(x.error)return;var n=x.count||0;show(n);save(n);window.dispatchEvent(new CustomEvent('trooth-unread-updated',{detail:{count:n}}))})}).catch(function(){})}
    ...
    window.addEventListener('trooth-auth-changed',function(){last=-1;schedule(250)});
